...
deploy(){
   ...
   ln -s /data/umapiserver1/umusic/config.json ${S_DEPLOY}/${S_DEPLOY_VERSION_TIME}/config.json
   ln -s /data/umapiserver1/umusic/bucket_read_write_policy.txt ${S_DEPLOY}/${S_DEPLOY_VERSION_TIME}/bucket_read_write_policy.txt
...
start()
{
  ...
  nohup java ${s_java_options} -Xloggc:gc.log -XX:+PrintGCTimeStamps -XX:-PrintGCDetails -cp dependency/*:classes ${s_main_class} > /data/logs/api.out 2>&1 &
